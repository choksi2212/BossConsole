...
internal object BrowserProfilePaths {
    ...
    fun isValidId(profileId: String): Boolean {
        val lengthOk = profileId.length <= MAX_PROFILE_ID_LENGTH
        return lengthOk && validProfileId.matches(profileId)
    }
    ...
    fun isTemporaryId(profileId: String): Boolean {
        val valid = isValidId(profileId)
        return valid && temporaryProfileId.matches(profileId)
    }
    ...
    fun delete(
        ...
        return if (isLinkOrReparsePoint(target)) {
            deleteEntry(target) && !Files.exists(target, NOFOLLOW_LINKS)
        } else {
            deleteTree(target)
        }
    }

    private fun deleteTree(target: Path): Boolean {
        var failed = false
        try {
            Files.walkFileTree(
                target,
                ...
                object : SimpleFileVisitor<Path>() {
                    override fun preVisitDirectory(
                        ...
                        if (dir != target && isLinkOrReparsePoint(dir, attrs)) {
    ...
    private fun isLinkOrReparsePoint(
        path: Path,
        cachedAttrs: BasicFileAttributes? = null,
    ): Boolean {
        if (Files.isSymbolicLink(path)) return true
        val attrs = cachedAttrs ?: readNoFollowAttributes(path)
        return attrs?.let { it.isSymbolicLink || it.isOther } ?: false
    }

    private fun readNoFollowAttributes(path: Path): BasicFileAttributes? =
        try {
