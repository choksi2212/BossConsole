...
chip code, the assertion that dragging ended fails. The original fix cancelled it.
The consolidated fix now preserves the gesture through reindexing and ends it on release.
...
The shared `detectTabDragGestures` adapter uses the session and notifies the current
callback once on cancellation. Stable tab-id/component keys preserve title and index
changes; release reads the current source index. This PR is stacked on #627, which
owns that stable-input behavior and the contributed idle-button ownership regression.
